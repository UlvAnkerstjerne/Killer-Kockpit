...
// v26 post insights: impression and reach metrics (post_impressions_unique,
// post_engaged_users, post_impressions) return (#100) on /{post-id}/insights.
// Only interaction breakdown metrics are available on this endpoint in v26.
// views / reach / engaged_users columns will remain null on this account.
const POST_METRICS_TO_REQUEST = 'post_reactions_by_type_total,post_clicks_by_type'
...
export async function fetchFbPostInsights(
  ...
  for (const item of data) {
    ...
    if (item.name === 'post_reactions_by_type_total' && rawVal && typeof rawVal === 'object') {
