...
export function gbpDb() {
  ...
  const from = (table: string) => {
    let operation = 'select', rows: Row[] = [], conflict = '', single = false, ignoreDuplicates = false
    let range: [number, number] | null = null
    const filters: Array<(row: Row) => boolean> = []
    const query = {
      select: () => query,
      eq: (key: string, value: unknown) => { filters.push(row => key === 'review.location_id' ? tables.gbp_reviews?.some(review => review.id === row.review_id && review.location_id === value) : row[key] === value); return query },
      not: (key: string, _op: string, value: unknown) => { filters.push(row => row[key] != null && row[key] !== value); return query },
      gte: (key: string, value: string) => { filters.push(row => row[key] != null && row[key] >= value); return query },
      lt: (key: string, value: string) => { filters.push(row => row[key] != null && row[key] < value); return query },
      ...
      upsert: (data: Row | Row[], options: { onConflict: string; ignoreDuplicates?: boolean }) => { operation = 'upsert'; rows = Array.isArray(data) ? data : [data]; conflict = options.onConflict; ignoreDuplicates = !!options.ignoreDuplicates; return query },
      ...
      then: (resolve: (value: { data: unknown; error: unknown }) => unknown) => {
        ...
        if (operation === 'insert' || operation === 'upsert') {
          matches = rows.map(data => {
            ...
            if (existing) { if (!ignoreDuplicates) Object.assign(existing, data); return existing }
  ...
  const rpc = async (_name: string, args: Row) => {
    ...
    if (_name === 'capture_gbp_review_health') return { data: null, error: null }
