...
vi.mock('@/lib/google/ads-sync', () => ({ runGoogleAdsSync: mocks.sync }))
import { GET, POST } from '@/app/api/google/ads/sync/route'
beforeEach(() => { vi.clearAllMocks(); vi.stubEnv('CRON_SECRET', 'test-secret'); mocks.sync.mockResolvedValue({ ok: true }) })
describe('Google Ads sync authorization', () => {
  it.each([undefined, 'Bearer wrong'])('rejects cron requests without the exact secret', async authorization => {
    const response = await POST(new NextRequest('http://localhost/api/google/ads/sync', { method: 'POST', headers: authorization ? { authorization } : {} }))
  ...
  it('fails closed when the server secret is absent', async () => {
    ...
    expect((await POST(new NextRequest('http://localhost/api/google/ads/sync', { method: 'POST' }))).status).toBe(500)
    expect(mocks.sync).not.toHaveBeenCalled()
  })
  it('allows the cron, disables response caching and returns failure HTTP status', async () => {
    mocks.sync.mockResolvedValue({ ok: false, errors: ['CLOUD_PROJECT_NOT_APPROVED_FOR_PRODUCTION'] })
    const response = await POST(new NextRequest('http://localhost/api/google/ads/sync', { method: 'POST', headers: { authorization: 'Bearer test-secret' } }))
