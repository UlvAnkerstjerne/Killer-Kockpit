 ...
 *   Includes a backfill pass for existing rows whose insight columns are null
 *   (handles the case where the cursor advanced past existing media before
 *   insights were populated).
 *
 * Idempotency:
...
async function syncIgOrganicDeep(db: Db, igAccountId: string, now: string): Promise<void> {
  ...
  try {
    ...
    // Backfill pass: fetch insights for existing rows whose insight columns are
    // null but published_at is within the lookback window. This handles the case
    // where the cursor advanced past existing media before insights were populated
    // (e.g., when the incremental `since` filter returns zero new items but
    // previously-inserted rows still need their insight columns filled).
    const { data: nullInsightRows } = await db
      .from('meta_ig_media')
    ...
    if (nullInsightRows && nullInsightRows.length > 0) {
      console.log(`[meta/sync] IG backfill pass: ${nullInsightRows.length} rows with null reach within lookback window`)
      for (const row of nullInsightRows) {
        const insights = await fetchIgMediaInsights(row.id, row.media_type)
        if (insights) {
          await db.from('meta_ig_media').update({
            reach:               insights.reach               ?? null,
            plays:               insights.plays               ?? null,
