 ...
 * 4. Retrieve current (non-superseded) Updates via two complementary paths:
 ...
 * 5. Deduplicate, sort by recency, cap at MAX_CONTEXT_UPDATES.
 * 6. Enrich with entity links + author names.
 * 7. Call queryBrain() AI module.
 * 8. Return { answer, sources } or { error }.
 *
 * Security
 * ────────
...
import type { BrainContextUpdate } from '@/lib/ai/brain-query'
...
export async function askBrain(
  ...
  // ── 5. Collect all superseded update IDs (for client-side exclusion) ──────
  ...
  // ── 10. Call AI ───────────────────────────────────────────────────────────
  const aiResult = await queryBrain(q, contextUpdates)
  if (!aiResult.ok) return { error: aiResult.error }

  // ── 11. Build sources for UI display ──────────────────────────────────────
  ...
  return { data: { answer: aiResult.answer, sources } }
}
